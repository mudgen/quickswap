...
contract QuickSwapRouter02 is IQuickSwapRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = QuickSwapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = QuickSwapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'QuickSwapRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = QuickSwapLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function removeLiquidity(
        ...
        address pair = QuickSwapLibrary.pairFor(factory, tokenA, tokenB);
        IQuickSwapERC20(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IQuickSwapPair(pair).burn(to);
    ...
    function removeLiquidityWithPermit(
        ...
        IQuickSwapERC20(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, QuickSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWETH(WETH).transfer(QuickSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'QuickSwapRouter: INVALID_PATH');
        amounts = QuickSwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'QuickSwapRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(QuickSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = QuickSwapLibrary.sortTokens(input, output);
            IQuickSwapPair pair = IQuickSwapPair(QuickSwapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, QuickSwapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
